    ...
    def update(self, bboxes):
        ...
        if self.args.asso=='hmiou':
            ...
            if self.args.sort_with_bec:
                bec_matrix = bec_score(detections, strack_pool, self.args.bec_num)
            else:
                bec_matrix = 0
            ...
            if not self.args.mot20:
                dists = matching.fuse_score(dists, detections)
            matches, u_track, u_detection = matching.linear_assignment(dists, thresh=self.args.match_thresh-np.max(bec_matrix*self.args.w_bec))
