...
class CMCComputer:
    def __init__(self, minimum_features=10, method="file"):
        ...
        # Same BoT-SORT CMC arrays
            ...
            for f_name in os.listdir("./cache/cmc_files/DanceTrack/"):
                ...
                self.file_names[tag] = f_name  # dancetrack0001->path
    ...
    def compute_affine(self, tag):
        if tag in self.cache:
            A = self.cache[tag]
            return A

        A = self.comp_function(tag)
        self.cache[tag] = A
        return A

    def _affine_file(self, tag):
        name, num = tag.split(":")
    ...
    def _affine_sift(self, frame, mask, tag):
        ...
        if len(good) > self.minimum_features:
            ...
            A, _ = cv2.estimateAffinePartial2D(src_pts, dst_pts, method=cv2.RANSAC)  # src原数组  dst目标数组
        else:
