...
class KalmanFilterNew(object):
    ...
    **Example**
    ...
    def __init__(self, dim_x, dim_z, dim_u=0):
        ...
        self.x = zeros((dim_x, 1))        # state  当前估计状态
        self.P = eye(dim_x)               # uncertainty covariance  当前状态协方差矩阵
        self.Q = eye(dim_x)               # process uncertainty  模型过程噪声矩阵
        self.B = None                     # control transition matrix
        self.F = eye(dim_x)               # state transition matrix  状态转移矩阵
        self.H = zeros((dim_z, dim_x))    # measurement function  观测矩阵
        self.R = eye(dim_z)               # measurement uncertainty  测量噪声矩阵
        ...
        # purposes
        self.K = np.zeros((dim_x, dim_z)) # kalman gain  卡尔曼增益
        self.y = zeros((dim_z, 1))
        self.S = np.zeros((dim_z, dim_z)) # system uncertainty 系统不确定性(用于求解卡尔曼增益)
        self.SI = np.zeros((dim_z, dim_z)) # inverse system uncertainty  系统不确定性的逆(用于求解卡尔曼增益)

        # identity matrix. Do not alter this.
        self._I = np.eye(dim_x)

        # these will always be a copy of x,P after predict() is called
        self.x_prior = self.x.copy()  # 先验预测值
        self.P_prior = self.P.copy()  # 先验状态协方差矩阵

        # these will always be a copy of x,P after update() is called
        self.x_post = self.x.copy()   # 后验预测值         
        self.P_post = self.P.copy()   # 后验状态协方差矩阵
        ...
        self.history_obs = []  # 历史检测框

        self.inv = np.linalg.inv  # 矩阵逆运算

        self.attr_saved = None
        self.observed = False   # 上一帧是否有检测
    ...
    def freeze(self):
        ...
        """
        self.attr_saved = deepcopy(self.__dict__)  # 保存所有的属性
    ...
    def unfreeze(self):  # 线性插值 ORU
        if self.attr_saved is not None:
            new_history = deepcopy(self.history_obs)  # 包含当前帧
            self.__dict__ = self.attr_saved  # 将KF所有属性设置为上一次存在检测框的状态
            # self.history_obs = new_history 
            self.history_obs = self.history_obs[:-1]  # 不包含当前帧
            occur = [int(d is None) for d in new_history]
            indices = np.where(np.array(occur)==0)[0]  # 返回不存在检测框的所有索引(帧数)
            index1 = indices[-2]
            index2 = indices[-1]
            # box1 = new_history[index1]  # 当前帧
            box1 = self.last_measurement
            x1, y1, s1, r1 = box1 
            w1 = np.sqrt(s1 * r1)
            h1 = np.sqrt(s1 / r1)
            box2 = new_history[index2]  # 上一次检测框
            x2, y2, s2, r2 = box2 
            w2 = np.sqrt(s2 * r2)
            h2 = np.sqrt(s2 / r2)
            time_gap = index2 - index1  # delta_t
            dx = (x2-x1)/time_gap
            dy = (y2-y1)/time_gap
            dw = (w2-w1)/time_gap 
            dh = (h2-h1)/time_gap
            for i in range(index2 - index1):
                ...
                s = w * h 
                r = w / float(h)
                new_box = np.array([x, y, s, r]).reshape((4, 1))  # 线性插值，生成虚拟检测框
                """
                ...
                """
                self.update(new_box)  # 再次更新
                if not i == (index2-index1-1):  # i + index1 != index2 - 1
                    self.predict()
    ...
    def update(self, z, R=None, H=None):
        ...
        # self.observed = True
        if not self.observed:  # 当前帧存在检测框，上一帧不存在检测框
            """
                Get observation, use online smoothing to re-update parameters
            """
            self.unfreeze()  # ORU 虚拟轨迹模拟
        self.observed = True
