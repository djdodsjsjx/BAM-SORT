...
class EmbeddingComputer:
    def __init__(self, dataset, test_dataset, grid_off, max_batch=8):
        self.model = None
    ...
    def compute_embedding(self, img, bbox, tag):
        ...
        for idx in range(0, len(crops), self.max_batch):
            ...
            with torch.no_grad():
                batch_embs = self.model(batch_crops)  # size*ch*w*h(例：5*3*384*128) => size*ch(例：5*2048)
    ...
    def initialize_model(self):
        if self.dataset == "mot17":
            if self.test_dataset:
                path = "pretrained/mot17_sbs_S50.pth"
            else:
                return self._get_general_model()
        elif self.dataset == "mot20":
            if self.test_dataset:
                path = "pretrained/mot20_sbs_S50.pth"
            else:
                return self._get_general_model()
        elif self.dataset == "dance":
            path = "pretrained/dance_sbs_S50.pth"
        else:
            path = "pretrained/mot20_sbs_S50.pth"
