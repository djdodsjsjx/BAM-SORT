...
class SparseTracker(object):
    def __init__(self, args, frame_rate = 30):
        ...
        self.layers = args.depth_levels ## MOT17:3  MOT20: 8  DanceTrack: 12
    ...
    def get_deep_range(self, obj, step):
        col = []
        for t in obj:
            lend = (t.deep_vec)[2]  # 深度
            col.append(lend)
        max_len, mix_len = max(col), min(col)
        if max_len != mix_len:
            deep_range = np.arange(mix_len, max_len, (max_len - mix_len + 1) / step)  # 深度划分
    ...
    def DCM(self, detections, tracks, activated_starcks, refind_stracks, levels, thresh, is_fuse):
        ...
        if len(track_mask) != 0:
            ...
            for dm, tm in zip(det_mask, track_mask):  # 相同距离分级进行匹配
                ...
                det_ = det_ + u_detection  # u_detection上一个距离阶段没有匹配的检测框
    ...
    def update(self, output_results, img_info, img_size):
        ...
        # track2.py
        # curr_img = img_info['raw_img']
        # img_h, img_w = img_info['height'], img_info['width']

        # 评估
        curr_img = cv2.imread('datasets/MOT20/train/' + img_info[4][0])
        ...
        for inew in u_detection:
            ...
            if track.score < self.det_thresh:
                continue
            track.activate(self.kalman_filter, self.frame_id)  # 轨迹激活
            activated_starcks.append(track)
        """ Step 5: Update state"""
