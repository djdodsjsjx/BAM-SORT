    ...
    def evaluate_ocsort(
        ...
        for cur_iter, (imgs, _, info_imgs, ids, raw_img) in enumerate(
            ...
            with torch.no_grad():
                ...
                if frame_id == 1:
                    ...
                    if len(results) != 0:
                        ...
                        result_filename_dets = os.path.join("exps/MOT17/{}/val".format(self.args.det_type), '{}.txt'.format(video_names[video_id - 1]))
            ...
            if outputs[0] is not None:
                ...
                # online_targets = tracker.update(raw_img.copy(), outputs[0], info_imgs, self.img_size)
                online_targets = tracker.update(bboxes_scores)
                online_tlwhs = []
                online_ids = []
                online_scores = []
                for t in online_targets:
            ...
            if cur_iter == len(self.dataloader) - 1:
                ...
                result_filename_dets = os.path.join("exps/MOT17/{}/val".format(self.args.det_type), '{}.txt'.format(video_names[video_id]))
                write_det_results(result_filename_dets, det_results)
        statistics = torch.cuda.FloatTensor([inference_time, track_time, n_samples])
        if distributed:
            data_list = gather(data_list, dst=0)
