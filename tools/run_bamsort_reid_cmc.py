...
from trackers.bamsort_tracker.bamsort_bec_atm_std_reid_cmc import OCSort
    ...
    for seq_name in test_seqs:
        ...
        for frame_ind in range(int(min_frame), int(max_frame)+1):
            if args.dataset == "dancetrack":
                tag = f"{seq_name[:-4]}:{frame_ind}"
            else:
                tag = f"{seq_name[0:8]}:{frame_ind}"
            ...
            online_targets = tracker.update(cur_dets, det_embs, tag)
