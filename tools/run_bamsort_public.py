...
def get_all_dirs_in_directory(directory):    
    # 使用 os.walk() 遍历指定文件夹及其子文件夹
    file_list = []
    for root, dirs, files in os.walk(directory):
        for dir in dirs:
...
def main(args):
    ...
    raw_path = "{}/{}/{}".format(args.raw_results_public_path, args.dataset, "test" if args.dataset_type == "test" else "train")  # 检测路径
    dataset = args.dataset

    total_time = 0
    total_frame = 0

    test_seqs = get_all_dirs_in_directory(raw_path)
    ...
    for seq_name in test_seqs:
        ...
        seq_trks = np.empty((0, 7))
        seq_file = os.path.join(raw_path, seq_name, "det", "det{}.txt".format(args.gt_type))
        seq_file = open(seq_file)
        results_filename = os.path.join(results_folder, "{}.txt".format(seq_name))
