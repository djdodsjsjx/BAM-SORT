...
if __name__ == '__main__':
    ...
    for split in SPLITS:
        ...
        #        'categories': [{'id': 1, 'name': 'pedestrian'}]}  # json格式
        out = {
            'images': [], 
            'annotations': [],  # 跟踪器相关参数
            'videos': [],
        ...
        for seq in sorted(seqs):
            ...
            seq_path = os.path.join(data_path, seq)  # datasets/MOT20/train/MOT20-01
            img_path = os.path.join(seq_path, 'img1')  # datasets/MOT20/train/MOT20-01/img1
            ann_path = os.path.join(seq_path, 'gt/gt.txt')  # datasets/MOT20/train/MOT20-01/gt/gt.txt
            images = os.listdir(img_path)  # 图片列表
            num_images = len([image for image in images if 'jpg' in image])  # half and half

            if HALF_VIDEO and ('half' in split):
                # image_range = [0, num_images // 2] if 'train' in split else \
                #               [num_images // 2 + 1, num_images - 1]  # 训练集和验证集各一半
                image_range = [0, num_images * 4 // 5 if 'train' in split else \
                               num_images * 4 // 5 + 1, num_images - 1] # 训练集8，验证集2
            else:
            ...
            if split != 'test':
                ...
                if CREATE_SPLITTED_ANN and ('half' in split):  # 跟踪器: train_half和val_half
                    ...
                    for o in anns_out:
                        fout.write('{:d},{:d},{:d},{:d},{:d},{:d},{:d},{:d},{:.6f}\n'.format(
                                    ...
                                    int(o[6]), int(o[7]), o[8]))
                    fout.close()
                if CREATE_SPLITTED_DET and ('half' in split):  # 检测框: train_half和val_half
                ...
                for i in range(anns.shape[0]):
                    ...
                    ann = {'id': ann_cnt,
                           ...
                           'conf': float(anns[i][6]),
                           'iscrowd': 0,
                           'area': float(anns[i][4] * anns[i][5])}  # 跟踪器参数
